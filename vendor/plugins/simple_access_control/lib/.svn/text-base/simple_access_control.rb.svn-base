...
#
# 2006-11-20  james.anderson@makalumedia.com  added @@verbose
...
module SimpleAccessControl
  ...
  def permission_required(rule = nil)
    if (@@verbose)
      puts("permission_required: rule: '#{rule}' logged_in?: #{logged_in?} admin?: #{current_user.is_admin?}")
    end
  ...
  def has_permission?(rule, user = nil)
    ...
    result = access_controller.process(rule, user)
    if (@@verbose)
      puts("has_permission?: rule: '#{rule}' user: [#{user.id}/#{user.roles.map{|r| r.title}.join','}] == #{result}")
    end
    result
  end
