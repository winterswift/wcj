# 2006-11-20  james.anderson@makalumedia.com added guard to run_initial_state_actions to ensure that
#  the designated initial state exists.
    ...
    module StateMachine              #:nodoc:
      ...
      module InstanceMethods
        ...
        def run_initial_state_actions
          ...
          unless initial
            fail(NoInitialState,
                 "initial state not defined among class states: #{self.class.name}: #{self.class.read_inheritable_attribute(:states)}.")
            end
        ...
        # 20070125 james.anderson@makalumedia.com : initialize if needed
        def current_state
          if (state = self.send(self.class.state_column))
            state.to_sym
          else
            set_initial_state()
