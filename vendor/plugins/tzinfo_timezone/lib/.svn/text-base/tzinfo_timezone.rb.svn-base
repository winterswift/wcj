# 2006-12-10  james.anderson@makalumedia.com
#   include canonical time zone designators in the cache and accept them as
...
class TzinfoTimezone
  ...
  def tzinfo
    ...
    # accept canonical designators as well as places
    @tzinfo = MAPPING[name] || name
    if String === @tzinfo
      ...
      unless @@zones
        ...
        each do |offset, *places|
          ...
          places.each do |place|
            ...
            # add canonical location - do this for each, since some groups
            # comprise locations in different primary places
            canonical_place = MAPPING[place]
            if ( canonical_place )
              unless ( @@zones_map[canonical_place] )
                zone = create(canonical_place, offset)
                @@zones << zone
                @@zones_map[canonical_place] = zone
              end
            else
              fail("no time zone designator for place: #{place}.")
            end
          end
        end
        @@zones.sort!
